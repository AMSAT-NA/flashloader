...
uint32_t Fapi_BlockErase(uint32_t Bank, uint32_t ulAddr, uint32_t Size) {
	uint8_t i, j, ucStartBank, ucEndBank, ucStartSector, ucEndSector;
	uint32_t EndAddr, status, sectorEnd;
	...
	for (j = ucStartSector; j < NUMBEROFSECTORS; j++) {
		sectorEnd = (uint32_t) flash_sector[j].start + flash_sector[j].length;
		if (EndAddr <= sectorEnd) {
			ucEndBank = flash_sector[j].bankNumber;
			ucEndSector = j;
			break;
		}
	}
